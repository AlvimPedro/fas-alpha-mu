...
for i = 2:N
    ...
    fo = fo.*...
         besseli(mu-1,2*mu*sqrt(delta(k-1)*r.^alpha(1)*rth^alpha(i))/(1-delta(k-1))).*...
         r.^(-1+0.5*alpha(1)*(mu+1)).*exp(-mu*r.^alpha(1)/(1-delta(k-1)));
     
     F = F + alpha(i)*mu*rth^(0.5*alpha(i)*(mu+1))*trapz(r,fo)*...
             exp(-mu*rth^alpha(i)/(1-delta(k-1)))*delta(k-1)^(0.5*(1-mu))./...
             (1-delta(k-1));
